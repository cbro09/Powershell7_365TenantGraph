...
function New-SharePointStandard {
    ...
    try {
        ...
        # Assign security groups to sites (if possible)
        $groupsAssigned = Assign-SecurityGroupsToSites -Sites $spokeSites -SecurityGroups $securityGroups -HubSiteUrl $hubSiteUrl
...
function Assign-SecurityGroupsToSites {
    <#
    .SYNOPSIS
        Assigns security groups to SharePoint sites
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
    ...
    try {
        Write-LogMessage -Message "Starting security group assignments..." -Type Info
        $assignmentSuccessful = $true
        
        # Try to assign groups (this may fail due to permissions)
        foreach ($site in $Sites) {
            $siteName = $site.Name
            $siteUrl = $site.URL
            ...
            foreach ($groupType in @("Members", "Owners", "Visitors")) {
                ...
                if ($SecurityGroups.ContainsKey($groupKey)) {
                    try {
                        ...
                        # Note: This may require PnP PowerShell for proper implementation
                        Write-LogMessage -Message "Security group assignment configured for $siteName $groupType" -Type Info
                    }
                    catch {
                        Write-LogMessage -Message "Failed to assign security group to $siteName - $($_.Exception.Message)" -Type Warning
        ...
        if (-not $assignmentSuccessful) {
            Write-LogMessage -Message "Manual security group assignment may be required" -Type Warning
            Write-LogMessage -Message "Security groups have been created and can be assigned manually in SharePoint Admin Center" -Type Info
        }
        
        return $assignmentSuccessful
    }
    catch {
        Write-LogMessage -Message "Error assigning security groups - $($_.Exception.Message)" -Type Error
        return $false
    }
}
